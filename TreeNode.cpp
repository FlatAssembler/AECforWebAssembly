...
public:
  ...
  TreeNode() {
	  basicDataTypeSizes["Integer32"]=4;
	  	...
	  	for (auto iterator=basicDataTypeSizes.begin(); iterator!=basicDataTypeSizes.end(); iterator++)
	  		if (iterator->first.find("Pointer")==std::string::npos) basicDataTypeSizes[iterator->first+"Pointer"]=4; //WebAssembly is 32-bit (pointers being 32 bits or 4 bytes long), unless somebody switches to the 64-bit mode (which is rarely done).
  ...
  int interpretAsACompileTimeConstant() {
	  ...
	  if (text=="+")
		  return children[0].interpretAsACompileTimeConstant()+children[1].interpretAsACompileTimeConstant();
	  if (text=="-")
		  return children[0].interpretAsACompileTimeConstant()-children[1].interpretAsACompileTimeConstant();
	  if (text=="*")
		  return children[0].interpretAsACompileTimeConstant()*children[1].interpretAsACompileTimeConstant();
	  if (text=="/")
		  return children[0].interpretAsACompileTimeConstant()/children[1].interpretAsACompileTimeConstant();
	  if (text=="?:")
		  return children[0].interpretAsACompileTimeConstant()?children[1].interpretAsACompileTimeConstant():children[2].interpretAsACompileTimeConstant();
	  if (text=="mod(")
		  return children[0].interpretAsACompileTimeConstant()%children[1].interpretAsACompileTimeConstant();
	  if (text=="<")
		  return children[0].interpretAsACompileTimeConstant()<children[1].interpretAsACompileTimeConstant();
	  ...
	  if (text=="=")
		  return children[0].interpretAsACompileTimeConstant()==children[1].interpretAsACompileTimeConstant();
	  std::cerr <<"Line " <<lineNumber <<", Column " <<columnNumber <<", Interpreter error: \"" <<text <<"\" isn't a valid token in a compile-time integer constant." <<std::endl;
	  return 0;
	  }
  static std::vector<TreeNode> parseVariableDeclaration(std::vector<TreeNode> input);
};
